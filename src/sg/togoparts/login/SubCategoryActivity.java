...
public class SubCategoryActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		Log.d("haipn", "sessionid:" + mSectionId + ", categoryid :" +  + mCategoryId); 
		createHeader();
		...
		mLvCategories.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view,
				...
				data.putExtra(PostAdActivity.SUB_CAT, mListSection.get(position).id);
		...
		GsonRequest<SubCategoryResult> myReq = new GsonRequest<SubCategoryResult>(Method.POST, String.format(
				Const.URL_GET_SUBCATEGORY, mSectionId, mCategoryId), SubCategoryResult.class,
				createProfileSuccessListener(), createMyReqErrorListener()) {
			protected Map<String, String> getParams()
					throws com.android.volley.AuthFailureError {
				Map<String, String> params = new HashMap<String, String>();
				params.put("session_id", Const.getSessionId(SubCategoryActivity.this));
				return params;
			};
		};
		queue.add(myReq);
	...
	private Listener<SubCategoryResult> createProfileSuccessListener() {
		return new Response.Listener<SubCategoryResult>() {
			...
			public void onResponse(SubCategoryResult response) {
				...
				if (response.Result != null && response.Result.Sub_category != null) {
					Log.d("haipn", "list sub category lenght:" + response.Result.Sub_category.size());
